// Package sampler implements F1: stratified sampling of workflow
...
// Config mirrors the `sample:` section of replaygate.yaml. Zero
...
type Config struct {
	...
	// "hash". The "hash" profile also needs a key, which is
	...
	// scan will page through before giving up; without a bound, discovery on a
	// large namespace never terminates on its own.
